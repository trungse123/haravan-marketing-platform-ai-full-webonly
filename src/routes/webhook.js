...
// IMPORTANT: Mount this router BEFORE express.json() in server.js
...
router.post('/haravan', async (req, res) => {
  try {
    ...
    const topic = req.header('X-Haravan-Topic') || payload?.topic || 'unknown';
...
async function handleOrder(payload) {
  ...
  const items = (o.line_items || []).map(li => ({
    ...
    fandom: li.vendor || '', qty: li.quantity, price: parseFloat(li.price || 0),
    discount_allocation: 0, cogs: 0
  }));
  const doc = await Order.findOneAndUpdate(
    { id_hrv: o.id },
    { $set: { name: o.name, created_at: o.created_at ? new Date(o.created_at) : new Date(),
      processed_at: o.processed_at ? new Date(o.processed_at) : null, financial_status: o.financial_status || '',
      gateway: o.gateway || '', total_price: parseFloat(o.total_price || 0), subtotal_price: parseFloat(o.subtotal_price || 0),
      total_discounts: parseFloat(o.total_discounts || 0), currency: o.currency || 'VND', source_name: o.source_name || '',
      utm_source: o.source_name || '', utm_medium: '', utm_campaign: '', items } },
...
async function handleProduct(payload) {
  ...
  await Product.findOneAndUpdate(
    { id_hrv: p.id },
    { $set: { title: p.title, sku: (p.variants && p.variants[0]?.sku) || '',
      cogs: 0, price: (p.variants && parseFloat(p.variants[0]?.price || 0)) || 0,
      status: p.status || 'active', updated_at: p.updated_at ? new Date(p.updated_at) : new Date() } },
    { upsert: true, new: true }
  );
}
